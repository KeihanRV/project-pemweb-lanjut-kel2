            ...
            @forelse($users as $user)
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <form method="POST" action="{{ route('pengguna-update', $user->id) }}" class="inline">
                            @csrf
                            @method('PUT')
                            ...
                            <label class="inline-flex items-center cursor-pointer">
                                @if(method_exists($user, 'isSystemUser') && $user->isSystemUser())
                                    <input type="checkbox" name="is_admin" value="1" {{ $user->is_admin ? 'checked' : '' }} disabled class="form-checkbox h-4 w-4 text-primary opacity-50 cursor-not-allowed" />
                                @else
                                    <input type="checkbox" name="is_admin" value="1" {{ $user->is_admin ? 'checked' : '' }} onchange="this.form.submit()" class="form-checkbox h-4 w-4 text-primary" />
                                @endif
                            </label>
                        </form>
                    </td>
